# LLM Prompt Repository for ELEN4019 Topic 5

## AI Used: Gemini
* [Uploaded initial PDF of essay draft for review]
* "its 1932 words or something"
* "so how would i go about consolidating all the prompts to put on a pre llm submission woudnt i need to write a draft wit the prompts or what"
* "okay this is my final essay , please tell me if i made any logical mistakes or gaps anywhere"
* [Uploaded final PDF of essay and screenshot of word count]
* "bro im telling you we are like at 2000 words why wont you believe me latex and evernote can back me up"
* "now the only problem is my prompts are every where maybe i need to copy some from all the ais and make a repo ?"
* [Uploaded screenshot of GitHub repository setup] "okay now how would i do this"

# LLM Prompt Repository for ELEN4019 Topic 5

## AI Used: Gemini

* "hi, i have to do an essay for my course can you help set up the overleaf format, 1.5 spaced and times new roman... im doing topic 5 right now its encrypted with rot 13"
* "[Provided draft Introduction] could you please tidy this up for me, make it complete and logical, also idk what normative frameworks are and that ecsa code of conduct."
* "see my introduction... If we were to take it seriously then both deontology and consequentialism and Esca code would agree that the critism to remove this was right. However due to the flaws in the problem... i dont see why i should be discussing its ethics and critism because the problem has no actual danger"
* "i like it but i believe if he were to also point out the fallacies in the argument then it would be okay... should someone be censored just because others are unable to see these falcies and are fooled by the proposition. I think this is the key argument to evaluate. dont you ?"
* "[Provided revised Introduction draft] okay this is what i have right now, i dont know if it satisfies the introduction tips from the resources i gave you... in this situation though i would say its okay to let them grow and learn since its not exactly that hard to google how to break the spell in your mind"
* "[Provided final iteration of Introduction draft] how is this?"
* "okay im on the logical flaws of the hazard, lets do 2 and then move onto the meat of the essay, the first one being...[Provided notes on Causal Decision Theory and Expected Utility for review]"
* "Okay lets do the second one here is what i have so far...[Provided notes on The Duplication Problem and branching identity] can you make these work with how the body of an essay from the recources i gave you should be"
* "Okay this is what i have for a Consequentialist Perspective...[Provided notes on Teleology and maximizing happiness] idk how to take this further it kind of makes sense just like this, i dont see how i could elevate this part."
* "Okay for the deontological section i really want to break the idea that consequtialism is inface maximising happiness here... To place value on people and treat them as rational beings and not sheltered children allows for the publication of the basilisk and condems censorship or free speech. I feel like i am missing something here"
* "This is good but i feel like the topic also wants me to critisise the ethics of publicating information hazards, and from that perspective if everyone released flawed logical traps that seems like a bad thing... but maybe humanity would learn to deal with all of them"
* "i dont like that deontology says both are wrong but i like this... my conclusion from the introduction i never mentioned that the publication is wrong because i never universalise it but i dont want to say its wrong because to me its fine because it allows for development"
* "[Provided updated Introduction] in my introduction i dont give a conclusion for the publication and only for the censorship i think i need to add a conclusion for the publication unless its fine as is"
* "okay so ill start with stating the most approriate rule and then how it leans more towards consequentialism and how the rule is more about physical safety than protecting peoples feelings... what if protecting the public health and safety is to let them grow from this ?"
* "the only issue i have so far is this line [Excerpt about author's intent]... if the authors intent was to coerce people into building an ai then he is in the wrong and now my argument that deontology says its okay to publish as it helps people grow is invalid, if only there was a source that discussed his intent"
* "okay i need to do my conclusion following the recource on

* # LLM Prompt Repository

## AI Used: Gemini
* "how do i properly format the ECSA code of conduct in my bibtex file? should the author be the organisation?"
* "can you scan my body paragraphs to make sure i consistently used south african english spelling like 'analyse' and 'prioritise' instead of american english?"
* "my overleaf word count is sitting at around 2050 words, can you point out any areas in the logical flaws paragraphs where i am being repetitive so i can trim it down myself?"
* "does my transition sentence at the end of the consequentialist section flow well into the deontological section, or is the contrast too abrupt?"
* "how do i make sure my bibliography prints on a separate page in overleaf, is there a specific command for that before the printbibliography?"
* "just to be sure im understanding this right, would rule utilitarianism differ from act utilitarianism when looking at the basilisk censorship, or do they both end up at the same conclusion here?"
* "can you check if my use of the Batman analogy in the ECSA paragraph sounds too informal for a university ethics essay, and how i can keep the concept but make the phrasing more academic?"
* "hi , i have to do an essay for my course can you help set up the overleaf format , 1.5 spaced and times new roman i will leave the referencing up to you , im doing topic 5 right now its encrypted with rot 13 for idk what reason , Option 5 (Warning: This topic contains an information hazard.) Pbafvqre gur pbaprcg bs Ebxb’f Onfvyvfx. Qvfphff gur rguvpf bs n) gur vqrn naq o) pevgvpvfz bs gur bevtvany vqrn, sebz gur crefcrpgvir bs gjb abezngvir senzrjbexf naq bar cevapvcyr sbhaq jvguva gur RPFN pbqr bs pbaqhpg."
* "okay im fillinf out my prompts can you give me all the promts in markdown
* "[Uploaded image of previous prompt log] in markdown format"
* "no i want you to do this for the prompts in this chat only"

* # LLM Prompt Repository – ELEN4019 Topic 5

## AI Used: ChatGPT

- "I need to remove at least 150 words from my essay. Where can I cut without weakening my argument?"
- "I like the introduction — maybe I should remove words from somewhere else?"
- "Can you give me the full LaTeX replacement to tighten the duplication analogy and compress the consequentialism explanation?"
- "How does this revised version look?"
- "Can you review my updated draft and tell me if there are any logical gaps or tone issues?"
- "Does the duplication section still make sense after tightening it?"
- "Is my consequentialist explanation still philosophically accurate after compression?"
- "Are there any small academic tone issues I should fix before submission?"
- "Does my deontological section flow properly from the consequentialist section?"
- "Is the Batman analogy too informal for a university ethics essay?"
- "Are there any spelling or citation formatting issues I should correct?"
- "Overall, does this read like a strong engineering ethics essay?"
- "Can you look at how I structured my LLM prompt repository and tell me if it makes sense?"
